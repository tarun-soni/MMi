...
class Doctor extends React.Component {
  ...
  async loadBlockchainData() {
    ...
    if (networkData) {
      const contract = new web3.eth.Contract(Patients.abi, networkData.address);
  ...
  async GetPatientStaticData() {
    ...
    //getting id from BC
    const displayingUid = await this.state.contract.methods
      .getPatientId(this.state._id)
      .call();
    const convertedId = Number(displayingUid);
    this.setState({ convertedId });
    //getting Patients static data from BC
    const showPatients = await this.state.contract.methods
      .getPatient(this.state._id)
      .call();
  ...
  async GetPatientDynamicData() {
    ...
    const MedData = await this.state.DoctorContract.methods
      .getData(this.state._showPresInput)
      .call();
        ...
        <div className="doctainer">
          <div className="row">
            <div className="col-md-8">
              <button className="btn btnops1" onClick={this.TogglePrev}>
                Previous Data
              </button>
              <button className="btn btnops2" onClick={this.ToggleMed}>
                Prescribe Medicines
              </button>
              {/* Display of Cards */}
              <br />
                ...
                <div className="DocCard center-block modal-lg">
                  ...
                  >
                    ...
                    <input
                      ...
                      ref={(input) => {
                        this.ReportInputContent = input;
                      }}
                    />
                    <hr />
                    <button type="submit" className="btn btnDoc">
                      Save Info
                    </button>
                    <button
                      type="submit"
                      className="btn btnDoc"
                      onClick={(event) => {
                        this.state.DoctorContract.methods
                          .addMed(
                            this.state._presid,
                            this.state._medInput,
                            this.state._reportInput
                          )
                    ...
                    <div className="modal-body">
                      <div className="input-group mb-3">
                        <input
                          ...
                          type="number"
                          ref={(input) => {
                            this.showPresInputContent = input;
                          }}
                          className="form-control"
                        ...
                        <div className="input-group-append">
                          <button
                            className="btn btnColl"
                            onClick={this.getDynamicData}
                          >
                            {' '}
                            Get Patient History
                          </button>
                  ...
                  <div className="cardInfo">
                    <Table className="text-dark" bordered>
                      <tbody>
                        ...
                        <tr>
                          {' '}
                          <td>Name:</td> <td>{this.state.showPatients[1]}</td>{' '}
                        </tr>
                        <tr>
                          {' '}
                          <td>Email:</td>
                          <td>{this.state.showPatients[2]}</td>{' '}
                        </tr>
                        <tr>
                          {' '}
                          <td>Blood Group:</td>
                          <td>{this.state.showPatients[3]}</td>{' '}
                        </tr>
                        <tr>
                          {' '}
                          <td>Gender:</td> <td>{this.state.showPatients[4]}</td>{' '}
                        </tr>
                        <tr>
                          {' '}
                          <td>ETH Addres:</td>{' '}
                          <td>{this.state.showPatients[5]}</td>{' '}
                        </tr>
                      </tbody>
                    </Table>
                  </div>
