...
class Lab extends Component {
  ...
  async GetPatientDynamicData() {
    ...
    const MedData = await this.state.DoctorContract.methods
      ...
      .call();
    console.log('ID', Number(MedData[0]));
    ...
    return (
      <div>
          ...
          >
            <div className="input-group mb-3">
              <input
                ...
                type="number"
                ref={(input) => {
                  this.showReportInputContent = input;
                }}
            ...
            <button
              className="btn btn-info hashSubmit"
              onClick={(event) => {
                ...
                let _rData = [...this.state.rData];
                let _sData = [...this.state.sData];
                for (let i = 0; i < this.state.memeHashArray.length; i++) {
                  _rData[
                    this.state.indexArrayofReport[i]
                  ] = this.state.memeHashArray[i];
                  // this.state.rData[
                  //   this.state.indexArrayofReport[i]
                  // ] = this.state.memeHashArray[i];
                  // this.state.sData[this.state.indexArrayofReport[i]] = 'true';

                  _sData[this.state.indexArrayofReport[i]] = 'true';
                }
                this.setState({ rData: _rData, sData: _sData });
        ...
        <div className="modal-lg hashSubmit">
          <form onSubmit={this.onSubmit}>{this.showCards()}</form>
        </div>
